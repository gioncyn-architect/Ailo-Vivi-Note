...
════════════════════════════════════════ */
function initApp() {
  try { setHeroDate(); }      catch(e) { console.warn('setHeroDate:', e); }
  try { initEmojiPicker(); }  catch(e) { console.warn('initEmoji:', e); }
  try { renderCalendar(); }   catch(e) { console.warn('renderCal:', e); }
  try { loadHome(); }         catch(e) { console.warn('loadHome:', e); }
  try { loadFinance(); }      catch(e) { console.warn('loadFinance:', e); }
  try { populateCatSelect(); }catch(e) { console.warn('populateCat:', e); }
  try { setDefaultDates(); }  catch(e) { console.warn('setDates:', e); }
  try { startAlarmWatcher(); }catch(e) { console.warn('alarm:', e); }
  // Aktifkan halaman home & nav
  navigateTo('home');
  const fpBtn = document.getElementById('fp-btn');
  if (fpBtn) {
    fpBtn.style.display = DB.get('fp_cred_id') ? 'flex' : 'none';
  }
}
